...
int decode_encode_hdlc(void*data, int *len, unsigned char *buf_hdlc, int remove, int pos)
{
  ...
  int ret;
	...
	if (!ret) {
		printk(KERN_INFO "Packet dropped due to bad HDLC coding/CRC\n");
		kfree(buf_9k);
		return -EINVAL;
	}
	if (remove)
	...
	uint16_t max_item;
		...
		if (*buf == EPST_PREFIX) {
			if (*(buf+1) == 0x26 || *(buf+1) == 0x27) {
				...
				printk(KERN_INFO"%s: id = 0x%x nv_num = %d \n", __func__, *(buf+1), nv_num);
				...
				printk(KERN_INFO"%s:id = 0x%x no default routing path\n", __func__, *(buf+1));
				return NO_DEF_ID;
		} else {
				/*printk("%s: not EPST_PREFIX id = 0x%x route to USB!!!\n", __func__, *buf);*/
				return NO_PST;
		}
...
int modem_to_userspace(void *buf, int r, int type, int is9k)
{
	...
	if (!req) {
		printk(KERN_INFO "There is no enough request to ARM11!!\n");
		return 0;
	}
	...
	/*in this case, cmd may reply error message*/
		...
		printk(KERN_INFO "%s:check error cmd=0x%x message=ox%x\n", __func__
		, value, *((uint8_t *)req->buf+1));
		if ((value == 0x27) || (value == 0x26)) {
			if (is9k == 1) {
...
static long htc_diag_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	/*	if (copy_from_user(&ctxt->is2ARM11, argp, sizeof(int)))
			return -EFAULT;*/
		printk(KERN_INFO "diag: fix me USB_DIAG_FUNC_IOC_AMR_SET\n");
		break;
	default:
		return -ENOTTY;
	}
			...
			size_t count, loff_t *pos)
{
	...
	if (ret < 0) {
		printk("%s: usb_diag_write error %d\n", __func__, ret);
		goto end;
	}
		ret = count;
			 ...
			 size_t count, loff_t *pos)
{
	...
	while (count > 0) {
		...
		if (driver->ch == NULL) {
			printk(KERN_INFO "%s: driver->ch == NULL", __func__);
			r = -EFAULT;
			break;
		} else if (ctxt->toARM9_buf == NULL) {
			printk(KERN_INFO "%s: ctxt->toARM9_buf == NULL", __func__);
			r = -EFAULT;
			break;
		}
	 ...
	 DUMP_PREFIX_ADDRESS, req->buf, req->actual, 1);
		...
		if (if_route_to_userspace(ctxt, cmd_id)) {
			...
			if (xpst_req) {
				...
				printk(KERN_INFO "%s No enough xpst_req \n", __func__);
		} else
			driver->nohdlc = 0;
#endif
		...
		unsigned intf, unsigned alt)
{
	...
	if (!dev)
		return -ENODEV;
	printk(KERN_INFO "diag_function_set_alt\n");
...
static int diag_set_enabled(const char *val, struct kernel_param *kp)
{
	...
	if (_context.cdev) {
		...
		printk("%s: sdio_diag_init_enable=%d\n", __func__, enabled);
	}
#endif
