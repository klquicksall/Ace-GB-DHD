					...
					struct device_attribute *attr, char *buf)
{
  ...
  memset(diag_trace, 0x00, sizeof(diag_trace));
	...
	print_hex_dump(KERN_DEBUG, "diag_debug_buf:", 16, 1, DUMP_PREFIX_ADDRESS, diag_debug_buf, 1026, 1);
			      ...
			      size_t count, loff_t *ppos)
{
	...
	if (pkt_type == MEMORY_DEVICE_LOG_TYPE) {
		...
		buf += 4;
			...
			if (driver->sdio_ch) {
#ifdef DIAG_DEBUG
				sz = sdio_write_avail(driver->sdio_ch);
				printk(KERN_INFO "sdio_write_avail = %d\n", sz);
#endif
...
static int diagchar_setup_cdev(dev_t devno)
{
	...
	if (err) {
		printk(KERN_INFO "dev_attr_diag_trace registration failed !\n\n");
