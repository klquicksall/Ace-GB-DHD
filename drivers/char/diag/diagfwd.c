...
static int diag_ch_sdio = 1;  /* 0 for DIAG_MDM, 1 for DIAG_LEGACY*/
...
int diagfwd_connect(void)
{
	...
	if (diag_ch_sdio) {
		if (driver->legacy_ch && !IS_ERR(driver->legacy_ch))
			...
			printk(KERN_INFO "diag:No data from SDIO without  USB LEGACY ch");
...
static void diag_smd_notify(void *ctxt, unsigned event)
{
	...
	case SMD_FUNC_OPEN_BT:
				/*BT DUN funciton*/
				printk(KERN_INFO "%s:bt check me\n", __func__);
	...
	case SMD_FUNC_CLOSE:
	default:
				printk(KERN_INFO "%s:diag smd is closed \n", __func__);
				break;
	}
...
void diag_smd_enable(smd_channel_t *ch, char *src, int mode)
{
	...
	printk(KERN_INFO "smd_try_open(%s): mode=%d\n", src, mode);
	...
	if (mode) {
		if (!driver->ch) {
			...
			if (!r)
				_ch = driver->ch;
				printk(KERN_INFO "_ch=%x: driver->ch=%xn", (unsigned int)_ch, (unsigned int)driver->ch);
		}
	} else {
		if (driver->ch) {
	...
	printk(KERN_INFO "smd_try_open(%s): r=%d _ch=%x\n", src, r, (unsigned int)_ch);
}
